...
## 1. Next Forcing: Causal World Modeling with Multi-Chunk Prediction / 下一个强迫：具有多块预测的因果世界建模

**Date**: 2026-06-09 | **arXiv**: [2606.11187v1](http://arxiv.org/abs/2606.11187v1) | **PDF**: [Link](http://arxiv.org/pdf/2606.11187v1)
...
自回归视频生成已成为世界行动模型 (WAM) 的强大范例。然而，现有方法存在训练收敛速度慢和收敛精度有限的问题，特别是在高帧速率下，因为训练监督仅限于当前块，没有关于未来动态的明确信号；由于迭代视频去噪，它们还面临推理缓慢的问题。在本文中，我们提出了 Next Forcing，这是一种用于因果世界建模的多块预测 (MCP) 框架，可实现更快的训练、更高的准确性和加速的推理。受大型语言模型中多标记预测的启发，Next Forcing 引入了 MCP 训练目标，该目标通过轻量级辅助 MCP 模块增强主模型，以同时对多个未来时间范围内的视频块（next$^1$、next$^2$、next$^3$ 块）进行去噪。这些 MCP 模块形成了跨预测深度的因果链，其中利用主模型多层融合的中间特征来预测未来动态，允许近期预测为更远未来的预测提供信息，并向主模型提供密集的多尺度时间监督。在训练过程中，MCP 模块显着加速收敛并提高收敛精度，尤其是在高帧速率下：在 50 fps 下，Next Forcing 在 5k 训练步数下比 LingBot-VA 实现了 93.1% 的相对改进，收敛速度提高了 2.3 倍，并在 RoboTwin 基准上建立了新的最先进结果（在 Clean/Random 上为 94.1/93.5%）。在推理时，可以保留 MCP 模块来与当前视频块并行预测下一个视频块，从而实现 2 倍的推理加速。 Next Forcing 还展示了 PhyWorld（评估视频生成中物理定律遵守情况的基准）的显着改进，以及一般视频预训练的 FVD 减少了 50% 以上。

</details>

---

## 2. AnyMod-LLVE: Low-Light Video Enhancement with Modality-Agnostic Inference / AnyMod-LLVE：具有模态无关推理的低光视频增强

**Date**: 2026-06-09 | **arXiv**: [2606.11186v1](http://arxiv.org/abs/2606.11186v1) | **PDF**: [Link](http://arxiv.org/pdf/2606.11186v1)
...
由于低照度条件下信息严重退化，低光视频增强（LLVE）仍然是一项具有挑战性的任务。最近的多模态方法通过结合事件流和红外图像等辅助模态显着提高了增强性能。然而，这些方法通常假设这些模式在推理时可用，这在现实场景中通常是不可行的。为了解决这个问题，在这项工作中，我们提出了 AMNet，一种用于 LLVE 的统一多模态框架，以支持灵活的模态不可知推理，其中辅助模态可能不可用。为了解决模态缺失的问题，我们引入了空间谱双门转换器，它可以学习辅助模态和 RGB 输入之间的对应关系，生成隐式辅助表示以支持鲁棒增强。此外，为了充分促进跨模态对应的学习，我们基于仅 RGB 数据集和合成辅助模态进行大规模多模态预训练。大量实验表明，AMNet 可以处理任意推理时间模态组合，并在模态缺失条件下展现出 LLVE 的卓越性能。代码和模型可在项目页面上找到。

</details>

---

## 3. Lip Forcing: Few-Step Autoregressive Diffusion for Real-time Lip Synchronization / 唇部强迫：用于实时唇部同步的少步自回归扩散

**Date**: 2026-06-09 | **arXiv**: [2606.11180v1](http://arxiv.org/abs/2606.11180v1) | **PDF**: [Link](http://arxiv.org/pdf/2606.11180v1)
...
基于扩散的唇形同步模型实现了强大的视觉质量和视听对齐，但全序列双向注意力和许多去噪步骤使得它们对于实时推理来说不切实际。据我们所知，我们提出了 Lip Forcing，这是第一个用于视频到视频 (V2V) 唇同步的自回归扩散方法，该方法将 14B 音频调节双向视频扩散教师提炼为因果学生。在推理时，学生只需两个去噪步骤即可生成每个块，无需推理时 CFG，从而实现实时唇形同步。特定于口型同步的教师轨迹分析揭示了 CFG 保真度同步权衡：无 CFG 预测有利于参考保真度，而 CFG 引导的预测有利于中间轨迹带内的同步。 Lip Forcing 将这一发现转化为三个分析衍生组件：Sync-Window DMD、两步推理计划和基于 SyncNet 的奖励。我们在两个学生级别上验证了唇部强迫，这两个级别都是从 14B 老师那里提炼出来的。 1.3B 学生以 31 FPS 进入实时流媒体，比同规模的双向模型快 17.6 美元\倍$。 14B 学生是用于 V2V 唇形同步的最大扩散模型，在可比较的参考保真度下，其运行速度比其教师快 39.8 美元\倍$。在两个尺度上，第一帧的时间都是亚毫秒级的，远低于每个扩散基线。

</details>

---

## 4. Flow-DPPO: Divergence Proximal Policy Optimization for Flow Matching Models / Flow-DPPO：流匹配模型的发散近端策略优化
...
## 5. AnimaSpark: A Feed-Forward Method for Animating Arbitrary 3D Objects / AnimaSpark：用于任意 3D 对象动画的前馈方法

**Date**: 2026-06-09 | **arXiv**: [2606.10988v1](http://arxiv.org/abs/2606.10988v1) | **PDF**: [Link](http://arxiv.org/pdf/2606.10988v1)
...
虽然生成式 AI 的最新进展极大地加速了静态 3D 模型创建工作流程，但与类别无关的 3D 动画的合成仍然是 3D 资产制作中的一个重大瓶颈。目前与类别无关的动画生成方法在推理速度、运动质量和遵守文本提示方面表现出严重的局限性，从而使该过程依赖于劳动密集型的手工艺术。为了应对这些挑战，本文介绍了 AnimaSpark，这是一种用于生成与类别无关的 3D 动画的新颖管道。我们的方法源于这样一个关键见解：对于 3D 世界中的许多基本运动，通常可以在二维子空间内有效地建模相应的联合变换。该管道首先将装配好的静态 3D 模型渲染为其网格和骨架的多层图像表示，随后将其输入视频生成模型。然后，我们对生成的视频采用关键点跟踪算法来捕获投影到相机观察平面上的骨骼关节的运动。在最后阶段，我们从这些跟踪的关键点中提取平面平移和旋转，并将它们从 2D 域提升到 3D 空间以对角色进行动画处理。综合评估表明，我们的方法在关键指标上实现了优于现有最先进技术的性能，包括文本运动对齐、运动质量和计算效率。

</details>

---

## 6. HarmoView: Harmonizing Multi-View Constraints for Identity-Consistent Video Generation / HarmoView：协调多视图约束以生成身份一致的视频
...
## 7. SCAIL-2: Unifying Controlled Character Animation with End-to-end In-Context Conditioning / SCAIL-2：将受控角色动画与端到端上下文条件统一起来
...
## 8. FadeMem: Distance-Aware Memory Consolidation for Autoregressive Video Diffusion / FadeMem：用于自回归视频扩散的距离感知内存整合
...
## 9. Kwai Keye-VL-2.0 Technical Report / 快手-VL-2.0技术报告
...
## 10. HiMem-WAM: Hierarchical Memory-Gated World Action Models for Robotic Manipulation / HiMem-WAM：用于机器人操作的分层内存门控世界动作模型
...
## 11. Making Time Editable in Video Diffusion Transformers / 在视频扩散变压器中使时间可编辑
...
## 12. BiWM: Advancing Open-Source Interactive Video World Models with Bidirectional Autoregression / BiWM：利用双向自回归推进开源交互式视频世界模型
...
## 13. Interpretable Temporal Facial-Region Motion Analysis for In-the-Wild Parkinson's Disease Video Classification / 用于野外帕金森病视频分类的可解释颞面部区域运动分析
...
## 14. Efficient-WAM: A 1B-Parameter World-Action Model with Low-Cost Future Imagination / Efficient-WAM：具有低成本未来想象力的 1B 参数世界行动模型
...
## 15. Latent Spatial Memory for Video World Models / 视频世界模型的潜在空间记忆
...
## 16. Echo-Memory: A Controlled Study of Memory in Action World Models / 回声记忆：行动世界模型中记忆的对照研究
...
## 17. Hybrid Robustness Verification for Spatio-Temporal Neural Networks / 时空神经网络的混合鲁棒性验证
...
## 18. MAVIS: Multi-Agent Video Retrieval via Structured Video Understanding / MAVIS：通过结构化视频理解进行多智能体视频检索
...
## 19. CineDance: Towards Next-Generation Multi-Shot Long-Form Cinematic Audio-Video Generation / CineDance：迈向下一代多镜头长格式电影音频视频生成
...
## 20. SwiftVR: Real-Time One-Step Generative Video Restoration / SwiftVR：实时一步生成视频恢复
...
## 21. CapRL++: Unified Reinforcement Learning with Verifiable Rewards for Dense Image and Video Captioning / CapRL++：统一强化学习，为密集图像和视频字幕提供可验证的奖励
...
## 22. LiteVSR: Lightweight Adaptation of Frozen Diffusion Transformers for Video Super-Resolution / LiteVSR：冷冻扩散变压器的轻量级适应，用于视频超分辨率
...
## 23. EgoTactile: Learning Grasp Pressure for Everyday Objects from Egocentric Video / EgoTactile：从以自我为中心的视频中学习日常物体的抓握压力
...
## 24. CP4D: Compositional Physics-aware 4D Scene Generation / CP4D：组合物理感知 4D 场景生成
...
## 25. Vision-Language Guided Hyperspectral Object Tracking via Semantics Fusion and Contextual Template Updating / 通过语义融合和上下文模板更新进行视觉语言引导的高光谱对象跟踪
...
## 26. Zero-Parameter Geometric Gating for Temporally Stable Low-Altitude UAV Video Semantic Segmentation / 用于时间稳定低空无人机视频语义分割的零参数几何门控
...
## 27. OmniGen-AR: AutoRegressive Any-to-Image Generation / OmniGen-AR：自回归任意图像生成
...
## 28. Ultra Flash: Scaling Real-Time Streaming Video Generation to High Resolutions / Ultra Flash：将实时流视频生成扩展到高分辨率
...
## 29. Driving Video Retrieval for Complex Queries with Structured Grounding / 通过结构化基础驱动复杂查询的视频检索
