...
## 1. PhysStream: Streaming Physics-Grounded Video Generation with Structured Scene Memory and Fine-Grained Motion Control / PhysStream：具有结构化场景内存和细粒度运动控制的基于物理的流式视频生成
...
## 2. SlotDiT: Object-Centric Representations for Diffusion Transformers / SlotDiT：扩散变压器的以对象为中心的表示
...
## 3. XPACE: Joint World and Action Modeling from Heterogeneous Experience / XPACE：来自异构经验的联合世界和行动建模
...
## 4. Video-HolmesV2: Can MLLMs Reason with Spatio-Temporal Audio-Visual Evidence in Long Videos? / Video-HolmesV2：MLLM 能否利用长视频中的时空视听证据进行推理？

**Date**: 2026-09-15 | **arXiv**: [2609.17248v1](http://arxiv.org/abs/2609.17248v1) | **PDF**: [Link](http://arxiv.org/pdf/2609.17248v1)
...
多模态大语言模型已经表现出令人印象深刻的视频理解能力，但它们对长篇叙述的推理能力往往被以视觉为中心的评估和低效的上下文处理所掩盖。现有的基准过度依赖视觉启发法，同时边缘化听觉线索，有效地将模型简化为绕过真正的跨模式推理的“沉默的观察者”。此外，标准密集采样会产生证据与上下文的权衡：增加捕获证据的帧不可避免地会导致注意力分散和令牌爆炸。为了弥补这些差距，我们推出了 Video-HolmesV2，这是一种专为深度视听耦合而设计的新颖基准。与之前的作品不同，它强制执行基于证据的评估，要求模型用精确的时空视听证据来证明答案的合理性，从而减少猜测和幻觉证据的混淆效应。为了支持这一点，我们引入：（1）多模型交叉验证管道以确保任务严谨性； （2）用于细粒度校准的时空证据感知指标。此外，我们提出了音频文本引导令牌压缩框架。通过将任务意图与听觉锚点融合，我们的方法提炼出高价值的推理线索，以减轻长上下文噪音。在我们的评估中，即使是强大的专有模型也能达到低于 60% 的准确率，而我们的方法优于同类开源全模型。

</details>

---

## 5. Not Another Text Benchmark: Putting the "Visual" Back in Visual Question Answering for Large Video Models / 不是另一个文本基准：将“视觉”放回大型视频模型的视觉问答中
...
## 6. GeoLAM: Learning Geometry-Grounded Latent Actions from Unlabeled Human Videos / GeoLAM：从未标记的人类视频中学习基于几何的潜在动作
...
## 7. Bi-FlowGS: Bridging Generative View Completion and Gaussian Geometry through Bidirectional Flow Co-Refinement / Bi-FlowGS：通过双向流协同细化桥接生成视图完成和高斯几何
...
## 8. VOR-Bench: A Human Perception-Driven Benchmark for Video Object Removal / VOR-Bench：人类感知驱动的视频对象去除基准
...
## 9. Multi-modal Knowledge Preserving Adapter for Embedding Backward Compatibility / 用于嵌入向后兼容性的多模式知识保存适配器
...
## 10. tcnerv:dual-domain temporal context modeling for implicit neural video compression / tcnerv：隐式神经视频压缩的双域时间上下文建模
...
## 11. VideoMM: Adaptive Macro-Micro Inference for Efficient Video MLLMs / VideoMM：用于高效视频 MLLM 的自适应宏微观推理
...
## 12. MDN-Control: Mask-Depth-Noise Guided Region Control for Multi-Subject Video Editing / MDN-Control：用于多主题视频编辑的掩模深度噪声引导区域控制
...
## 13. Semantic-Aware Neural Video Codec for Error-Resilient Low-Latency Transmission / 用于容错低延迟传输的语义感知神经视频编解码器
...
## 15. LynnReal-Omni: Native multi-modal Video Generation for Agentic Visual Workflows / LynnReal-Omni：用于代理视觉工作流程的本机多模式视频生成
...
## 16. VC-Attention: Value Smoothing and Softmax Casting for Low-bit Attention / VC-Attention：低位注意力的值平滑和 Softmax 铸造
...
## 17. VideoScout: Learning Agentic Active Exploration with Adaptive Reasoning Pacing for Long Video Understanding / VideoScout：通过自适应推理节奏学习代理主动探索以实现长视频理解
...
## 18. DIDO: Distilling Interaction-Centric Dynamics into One-Step Denoising for World Action Models / DIDO：将以交互为中心的动力学提炼为世界行动模型的一步去噪
...
## 19. CatchMonitor: a machine learning system for automated fish discard quantification / CatchMonitor：用于自动鱼类丢弃量化的机器学习系统
...
## 20. BVB: Benchmarking Agentic Video Understanding via Programmatic Reconstruction in Blender / BVB：通过 Blender 中的编程重建对代理视频理解进行基准测试
...
## 21. MarKey: Marginal Utility Guided Greedy Keyframe Selection for Long Video Understanding / MarKey：用于长视频理解的边际效用引导贪婪关键帧选择
...
## 22. Query-Conditioned Spherical Centroid Aggregation for Multimodal Retrieval / 用于多模态检索的查询条件球形质心聚合
...
## 23. MoVT: Video-Augmented Motion Tokenizer for Text-to-Motion Generation / MoVT：用于生成文本到动作的视频增强运动标记器
...
## 24. World-Action Models for Robot Learning and Control: A Survey / 机器人学习和控制的世界行动模型：调查
...
## 25. Zero-shot video highlight detection based on text descriptions and synthetic images / 基于文本描述和合成图像的零样本视频亮点检测
...
## 26. CrossDistill: Balancing Quality and Diversity via Trajectory-Level Hybrid Few-Step Distillation / CrossDistill：通过轨迹级混合少步蒸馏平衡质量和多样性
...
## 27. DynEoMT: Learning Object Dynamicity from Online Segmentation Queries / DynEoMT：从在线分割查询中学习对象动态性
