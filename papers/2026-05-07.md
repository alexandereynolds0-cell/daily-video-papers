...
## 5. Audio-Visual Intelligence in Large Foundation Models / 大型基础模型中的视听智能
...
视听智能 (AVI) 已成为人工智能的核心前沿，它连接听觉和视觉模式，使机器能够在多模式现实世界中感知、生成和交互。在大型基础模型时代，音频和视觉的联合建模变得越来越重要，即不仅为了理解，而且为了跨动态、时间接地信号的可控生成和推理。 Meta MovieGen 和 Google Veo-3 等最新进展凸显了工业界和学术界对从海量多模态数据中学习的统一视听架构的日益关注。然而，尽管进展迅速，文献仍然支离破碎，涉及不同的任务、不一致的分类法和异构的评估实践，阻碍了系统比较和知识整合。这项调查首次从大型基础模型的角度对 AVI 进行了全面审查。我们建立了一个统一的分类法，涵盖了 AVI 任务的广泛领域，从理解（例如，语音识别、声音定位）到生成（例如，音频驱动的视频合成、视频到音频）和交互（例如，对话、体现或代理界面）。我们综合了方法论基础，包括模态标记化、跨模态融合、自回归和基于扩散的生成、大规模预训练、指令对齐和偏好优化。此外，我们还策划代表性数据集、基准和评估指标，提供跨任务系列的结构化比较，并确定同步、空间推理、可控性和安全性方面的开放挑战。通过将这个快速扩展的领域整合为一个连贯的框架，本次调查旨在为未来大规模 AVI 研究提供基础参考。

</details>

---

## 6. WorldJen: An End-to-End Multi-Dimensional Benchmark for Generative Video Models / WorldJen：生成视频模型的端到端多维基准
