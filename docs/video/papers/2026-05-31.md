...
自回归图像和视频生成器接受教师强制历史记录的训练，但必须在推理时从自己生成的前缀中进行采样，这使得它们容易受到曝光偏差和前缀漂移的影响。现有的补救措施要么修改训练，要么应用主要针对外部语义条件（例如类标签或文本提示）的采样时间指导，而不是测试下一步预测是否为生成的前缀本身提供强大的后验支持。我们提出了视觉前缀指导（VPG），这是一种用于自回归图像和视频生成的免训练推理时间指导方法。 VPG 通过将生成的前缀下的模型输出与损坏的前缀下的输出进行对比，然后将逻辑外推到加强生成的前缀的后验支持的候选者，来改进下一步预测。在使用 VAR 进行类条件图像生成、使用 Infinity 进行文本到图像生成以及使用 InfinityStar 进行文本到视频生成时，VPG 无需重新训练基本模型即可提高生成质量，将 VAR 上的 FID 平均减少 0.36，并提高图像和视频生成的基准性能。
...
自监督视频以对象为中心的学习 (OCL) 旨在发现不同的对象并跨时间关联它们，而自监督多对象跟踪 (MOT) 则侧重于关联预定义的对象检测或分割。尽管循环一致性 (CC) 在 MOT 中得到了很好的应用，但它不能天真地或明确地应用于 OCL 的潜在时隙空间。与 MOT 中的确定性和理想对象表示不同，由于场景分解不唯一，OCL 槽本质上是随机的且不明确的。在时隙上强制执行显式循环一致性 (ECC) 会强制执行严格的均值搜索。这严重惩罚了探索替代但同样有效的分解的模型，从而导致特征崩溃。为了解决这个困境，我们提出了 \textit{隐式循环一致性（ICC）}，它将循环一致性约束从限制性槽空间转移到连续重建流形，鼓励槽在共同解释视觉场景方面达成软共识，而不是强制严格的点对点特征对齐。对复杂视频 OCL 基准的大量实验表明，ICC 避免了功能崩溃，并且性能优于 ECC 基准。我们的源代码、模型检查点和训练日志在 https://github.com/Genera1Z/ICC 上提供。
