...
使用强化学习 (RL) 训练多轮代理工作流程使大型语言模型能够执行复杂的推理、使用外部工具以及进行单轮设置之外的迭代搜索。然而，多轮强化学习训练仍然高度不稳定，随着轮数的增加，通常会导致性能严重下降。通过理论分析，我们确定了三个紧密耦合的不稳定来源：推出训练环境不匹配、稀疏终端奖励下的弱轮级信用分配以及在不同策略版本下优化短期和长期轨迹时的异步策略漂移。我们证明这些问题在扁平轨迹优化中具有共同的结构根源，并通过统一的反向转弯公式来解决这些问题。我们提出逆向策略优化（RTPO），它将多轮部署组织为稀疏反向树，并以时间逆序执行轮级策略更新，使每个决策与其下游延续保持一致。 RTPO 可实现因果一致的轮级信用分配和策略上的延续，以控制异步漂移。我们提供的理论保证表明，RTPO 消除了所提出的轮次级公式下的上下文不匹配和异步漂移，减少了信用偏差，并收敛到递归最优性。多回合智能体 RL 基准实验表明，RTPO 在轨迹和回合水平基线上分别提高了 21.50% 和 10.76%，突显了其支持工具使用代理更稳定训练的潜力。
