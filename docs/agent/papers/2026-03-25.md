...
大型语言模型 (LLM) 与检索增强生成 (RAG) 和知识图相结合，为与管道和仪表图 (P&ID) 等工程图交互提供了新的机会。然而，直接使用法学硕士处理原始图像或智能 P&ID 文件通常成本高昂、效率低下，并且容易产生幻觉。这项工作介绍了 ChatP&ID，这是一个代理框架，可以使用图形检索增强生成 (GraphRAG)（我们将其称为工程图的 GraphRAG 范式）与 P&ID 进行基础且经济有效的自然语言交互。以 DEXPI 标准编码的智能 P&ID 被转换为结构化知识图，作为 LLM 代理基于图的检索和推理的基础。这种方法可以可靠地查询工程图，同时显着降低计算成本。跨商业 LLM API（OpenAI、Anthropic）的基准测试表明，与直接摄取智能 P&ID 文件相比，基于图形的表示比原始图像输入的准确性提高了 18%，并将令牌成本降低了 85%。虽然小型开源模型仍然难以解释知识图格式和结构化工程数据，但将它们与 VectorRAG 和 PathRAG 集成可将响应准确性提高高达 40%。值得注意的是，GPT-5-mini 与 ContextRAG 相结合实现了 91% 的准确率，每个任务的成本仅为 0.004 美元。由此产生的 ChatP&ID 界面可实现与复杂工程图的直观自然语言交互，并为危险和可操作性研究 (HAZOP) 和多智能体分析等人工智能辅助过程工程任务奠定基础。
