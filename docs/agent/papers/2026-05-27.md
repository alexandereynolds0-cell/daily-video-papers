...
大型语言模型 (LLM) 已发展成为在现实世界任务中与用户协作的交互式代理。在此类环境中，有效的协作越来越依赖于对用户的理解，而不仅仅是明确说明的内容，因为用户意图通常反映在碎片化的日常交互中，并且需要个性化建模和主动交互。然而，现有的代理基准主要评估推理和工具使用，很大程度上忽视了在现实场景中推断和利用用户偏好的挑战。为了解决这一差距，我们引入了 VitaBench 2.0，这是一个用于评估长期用户交互中的个性化和主动代理行为的基准。在 VitaBench 2.0 中，任务被组织为单个用户的时间顺序序列，其中偏好嵌入在碎片化和异构的交互中。成功完成任务需要代理不断从这些交互中提取、利用和更新用户偏好。我们通过要求智能体识别缺失信息并在做出决策之前主动从用户或环境中获取信息的任务来进一步评估主动性。为了支持系统分析，我们提供了可扩展的内存接口，可以跨不同内存架构进行受控比较。我们对一系列不同的前沿专有和开源法学硕士进行了基准测试。结果表明，即使对于最先进的模型，现实世界的个性化仍然具有很大的挑战性，揭示了当前能力和实际要求之间的巨大差距。广泛的分析进一步揭示了当前智能体在现实世界个性化决策中的故障模式和能力瓶颈，为未来模型的改进提供了见解。
...
## 34. FOUND-IT: Foundation-model-first Task-driven 3D Scene Graphs with Granularity on Demand / FOUND-IT：基础模型优先、任务驱动的 3D 场景图，具有按需粒度
...
## 35. Micro-Swarm Locomotion Optimization in Dynamic Flow using Multi-Objective Multi-Agent Reinforcement Learning / 使用多目标多智能体强化学习的动态流中的微群运动优化
