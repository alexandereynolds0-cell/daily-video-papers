...
多模式多智能体系统（MM-MAS）因其跨不同模式实现复杂推理和协调的能力而受到越来越多的关注。 As these systems continue to expand in scale and functionality, investigating their potential vulnerabilities has become increasingly important.然而，现有的多智能体系统中对抗性攻击的研究主要集中在孤立的智能体或单模态设置上，而 MM-MAS 的漏洞在很大程度上没有得到充分的探索。为了弥补这一差距，我们引入了 HAM$^{3}$，这是一种用于多模式多代理系统的分层攻击框架，它将攻击分解为三个互连的层。具体来说，在感知层，HAM$^{3}$ 通过扰乱视觉输入、文本输入及其融合的视觉文本表示来发起攻击。在通信层，它执行通信级攻击，破坏消息内容和交互拓扑，例如操纵共享上下文或通信链路来扭曲集体信息流。在推理层，它进行推理级攻击，干扰每个智能体的认知管道，使推理轨迹产生偏差，最终损害最终决策。我们通过基于不同推理范式（包括 ReAct、Plan-and-Solve 和 Reflexion）构建的多智能体系统，在 GQA 基准上评估 HAM$^{3}$。实验表明，我们的框架的攻击成功率高达78.3%，其中推理层攻击最为有效。 More than half of the successful attacks lead multiple agents to produce consistent errors. These findings offer valuable insights for building more robust and interpretable multi-agent intelligence.
...
整体扩展是通向 AGI 的唯一途径吗？本文挑战了单纯扩展单个模型就足以实现通用人工智能的教条。相反，我们将代理人工智能视为掌握现实世界任务的复杂、异构分布的必要范例。 Through rigorous theoretical derivations, we contrast the optimization constraints of monolithic learners against the efficiency of Agentic systems, progressing from simple routing mechanisms to general Directed Acyclic Graph (DAG) topologies.我们证明 Agentic AI 实现了指数级优越的泛化能力和样本效率。 Finally, we discuss the connection to Mixture-of-Experts, reinterpret the instability of current multi-agent frameworks, and call for greater research focus on Agentic AI.
