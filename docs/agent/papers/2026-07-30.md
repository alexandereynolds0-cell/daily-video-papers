...
模拟中的自我博弈会产生大规模的稳健驾驶策略。这种行为的演示是使用特权矢量化观察（例如精确的姿势和速度）进行的，甚至对于被遮挡的智能体也是如此。这假设感知已得到解决，并引入了与从以自我为中心的摄像机的视角驾驶的部署代理的部分观察的表示差距。一个常见的解决办法是将特权政策提炼给摄像头输入的学生，让学生模仿自己的观点无法证明的决定。相反，我们将视角自我对弈作为一种实用的训练制度。我们引入了 Pictura，这是一种 GPU 加速的多智能体驾驶模拟器，它在每一步中呈现每个智能体的以自我为中心的视图，从而从源头上减少表示差距。 Pictura 在单个 H100 上支持高达 500K 代理步骤/秒（200 万张图像/秒）。使用 Pictura，我们通过使用普通 PPO 进行自我对弈来训练 Alberti。这是第一个直接从透视图像训练的大规模驾驶自我对弈策略，无需特权观察。训练跨越 50B 个智能体步骤，行驶约 3500 万公里。它的驾驶性能接近其特权矢量化代理，并将零样本传输到在 Pictura 中重新渲染的 Waymo 开放运动数据集布局，其性能优于特权矢量化代理。项目页面：https://valeoai.github.io/Pictura/
...
## 28. WorkSurface-Bench: Benchmarking Enterprise Agents on Multi-Surface Knowledge Routing / WorkSurface-Bench：在多表面知识路由上对企业代理进行基准测试
...
## 29. Cooperative Multi-UAV Navigation in Complex Environments via Systematic Multi-Agent Deep Reinforcement Learning / 通过系统多智能体深度强化学习实现复杂环境中的多无人机协作导航
...
## 30. SGTP: Sampling-based Game-Theoretic Planning for Real-Time Multi-Vehicle Autonomous Racing / SGTP：基于采样的实时多车辆自动驾驶博弈论规划
...
自主多车赛车需要在激烈的交互中实时规划多样化的竞争行为。现有的规划者常常难以平衡战略多样性和计算效率。为了应对这一挑战，我们提出了基于采样的博弈论规划 (SGTP)，这是一种实时框架，它将博弈论推理与控制序列和动态推出的 GPU 加速采样相结合。使用游戏感知成本对采样轨迹进行排名，以捕获竞争性交互并生成不同的赛车行为。然后，我们的规划人员通过明确执行赛道边界和动态防撞约束来执行可行性选择，确保赛车策略之间安全可靠的转换。对具有挑战性的赛道进行的广泛模拟表明，SGTP 在高度互动的比赛中实现了 95.24% 的获胜率和 99.35% 的任务完成率，多个迭代求解步骤的平均计算时间为 0.095 s。我们还展示了 SGTP 在多达 10 个代理的大规模场景中的成功应用。我们发布了代码并提供了多智能体自主赛车算法的开源基准，以促进未来的研究。项目页面：https://sgtp-racing.github.io/。

</details>

---

## 31. Kimi K3: Open Frontier Intelligence / Kimi K3：开放前沿智能
...
## 32. Keep It InMind: Benchmarking the Implicit-Association Blind Spot in Agent Memory / 牢记这一点：对代理内存中的隐式关联盲点进行基准测试
...
## 33. Energy Constrained Hierarchical Underwater Monitoring via Local Multi-Agent RAG / 通过本地多代理 RAG 进行能量受限的分层水下监测
...
## 34. INS-ActBench: A Comprehensive Benchmark for Assessing Professional Actuarial Capability of Large Language Models / INS-ActBench：评估大型语言模型专业精算能力的综合基准
...
## 35. A New Role for Relevance: Guiding Corpus Interaction in Agentic Search / 相关性的新角色：在代理搜索中指导语料库交互
