...
## 4. Integrating AI into Requirements Quality Learning in Software Engineering Education: A TPACK-Guided Empirical Study / 将人工智能融入软件工程教育中的需求质量学习：TPACK 引导的实证研究
...
## 5. AgenticASR: Refining Speech Recognition in Real-World Scenarios via an Agentic Approach / AgenticASR：通过代理方法改进现实场景中的语音识别
...
## 6. Group-Reflective Self-Distillation for Agentic Reinforcement Learning / 用于代理强化学习的群体反思自蒸馏
...
## 7. SemPIC: Learning Semantic Position-Independent KV Caches / SemPIC：学习语义位置无关的 KV 缓存
...
## 8. Contrastive Reinforced Policy Optimization via Privileged Self-Distillation / 通过特权自蒸馏的对比强化策略优化
...
## 9. SKIMIX: Multi-Agent Harness-Time Scaling with Skill Mixture for Dynamic Harness Engineering / SKIMIX：动态线束工程的多智能体线束时间缩放与技能混合
...
人工智能代理越来越依赖于大型技能库，但选择、组合和维护技能仍然很困难。我们提出了 SKIMIX，一个多智能体框架，其中具有不同技能组合的智能体通过迭代细化进行协作。 SKIMIX 结合了基于嵌入的技能检索、子模块抗稀释路由和自适应技能进化。在六个推理基准中，多智能体协作极大地改善了开放式数学推理，但在多项选择任务上提供的收益有限或为负。代理数量缩放是非单调的，大多数改进出现在第一轮细化期间。这些结果表明，任务特征决定了技能水平集成是否有助于并为可扩展代理设计提供实用指导。

</details>

---

## 10. Share the Judge, Learn the Deferral: Where Specialization Helps LLM Evaluation / 分享法官，了解延期：专业化有助于 LLM 评估
...
## 11. MARS-RA: Rank Aggregation for Credit Assignment via Multimodal Comparisons in Embodied Multi-Agent Cooperation / MARS-RA：通过具体多代理合作中的多模式比较进行信用分配的排名聚合
...
## 12. $Σ$-Mem: An Online Reliability Memory for LLM-based Multi-Agent Systems / $Σ$-Mem：基于 LLM 的多代理系统的在线可靠性存储器
...
## 13. MMHBench: A Multi-Perspective Benchmark for Mental Health Understanding in Long-Form Videos / MMHBench：长视频中心理健康理解的多视角基准
...
## 14. An Empirical Study of Coordination Mode as the First-Class Citizen in From-Scratch Multi-Agent Coding / 从头开始多智能体编码中作为一等公民的协调模式的实证研究
...
## 16. Safety-Gated Agentic Supervisory Control on a Coupled Distillation Benchmark: Regime Map, Auditable Gate, and Co-Design Findings / 耦合蒸馏基准的安全门控代理监督控制：制度图、可审计门和协同设计结果
...
## 17. MECA: A Mechanism-Centered Agent for Constructing Well-Specified and Valuable Mathematical Conjectures / MECA：一种以机制为中心的代理，用于构建明确且有价值的数学猜想
...
## 18. Albilich: Steerable Proof-State Orchestration for LLM-Based Mathematical Research with CAS Integration / Albilich：基于 LLM 的数学研究与 CAS 集成的可引导证明状态编排
...
## 19. SpatialCLI: Learning to Reason With Spatial Tools, Then Without Them / SpatialCLI：学习使用空间工具进行推理，然后不使用它们
...
## 20. RefineSVG: Visual Feedback-Driven Reinforcement Learning for Image-to-SVG Generation / RefineSVG：用于图像到 SVG 生成的视觉反馈驱动的强化学习
...
## 21. HALO: Heterogeneous Admission through Localized Obligations for Safe Agentic Execution / HALO：通过本地化义务实现异构准入以实现安全代理执行
...
## 23. What makes prompts a graph: necessary and sufficient conditions for prompt graph engineering / 提示图的构成要素：提示图工程的充要条件
...
## 24. Evaluating Agentic Bioinformatics through Function, Evidence, and Validation / 通过功能、证据和验证评估代理生物信息学
...
## 25. ThreatForest: Multi-Agent Attack Tree Generation with Pluggable TTP Framework Mapping / ThreatForest：使用可插入 TTP 框架映射生成多代理攻击树
...
## 26. Automated Transcript Analysis for Detecting Flaws in Agentic Benchmarks / 自动转录分析用于检测代理基准中的缺陷
...
## 27. Skill Use or Skill Theater? Evaluating the Reasoning Backroom in Skill-Augmented Language Agents / 技能运用还是技能剧场？评估技能增强语言代理的推理后台
...
## 28. Leveraging Trajectory Graphs for Pre-Execution Error Diagnosis in Agentic LLM Systems / 利用轨迹图进行代理 LLM 系统中的执行前错误诊断
...
## 29. Rethinking Artificial Intelligence in Medical Imaging: Assumptions, Reality, and Reframing / 重新思考医学影像中的人工智能：假设、现实和重构
...
## 30. VideoCoCo: Code-as-CoT for Physically-Consistent Video Generation via an Agentic Dual-Engine System / VideoCoCo：Code-as-CoT，通过代理双引擎系统生成物理一致的视频
...
## 31. Partner Capability Estimation for Task-Agnostic Adaptation in Ad-Hoc Teamwork / 临时团队合作中任务无关适应的合作伙伴能力估计
...
## 32. AgentMap: Joint Equivalence and Subsumption Discovery for Ontology Matching / AgentMap：本体匹配的联合等价和包含发现
