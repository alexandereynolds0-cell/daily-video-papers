...
## 9. Incentive-Aligned Vehicle-to-Vehicle Energy Trading via Nash-Integrated Multi-Agent Reinforcement Learning / 通过纳什集成多代理强化学习进行激励协调的车辆间能源交易
...
评估软件工程能力已成为现代大语言模型（LLM）的核心组成部分；然而，阻碍进一步扩展的关键瓶颈不在于缺乏高质量的解决方案，而在于缺乏高质量的测试套件。测试套件对于合成程序修复轨迹和在强化学习中提供精确的反馈信号都是不可或缺的。不幸的是，由于成本高、注释难度大，长期以来高质量的测试套件很难获得，而法学硕士自动生成的测试套件往往比较肤浅，缺乏足够的判别力。作为构建高质量测试套件的第一步，我们引入了 SWE-Mutation，这是用于评估 LLM 生成的测试套件的基准。该基准测试通过引入系统变异的解决方案来表征测试套件，这些解决方案试图“欺骗”测试套件并通过验证。我们进一步提出了一个与语言无关的代理框架，用于自动生成复杂的突变体。我们的基准测试由源自 800 个原始实例的 2,636 个变异变体组成，并包括跨越九种编程语言的多语言子集。对七个法学硕士的实验表明，即使是 DeepSeek-V3.1 也只能实现 10.20% 的验证率和 36.15% 的检测率，凸显了当前法学硕士的不足。此外，我们的代理突变策略增强了真实性，与传统方法相比，平均检测率从 71.04% 降低到 39.81%。这些发现暴露了当前法学硕士生成可靠和有区别的测试套件的能力持续存在的缺陷。
...
## 18. Blind Spots in the Guard: How Domain-Camouflaged Injection Attacks Evade Detection in Multi-Agent LLM Systems / 防护中的盲点：域伪装注入攻击如何逃避多代理 LLM 系统中的检测
...
## 21. MAVEN: A Multi-stage Agentic Annotation Pipeline for Video Reasoning Tasks / MAVEN：用于视频推理任务的多阶段代理注释管道
...
## 22. ACC: Compiling Agent Trajectories for Long-Context Training / ACC：编译代理轨迹以进行长上下文训练
...
## 23. Toward AI VIS Co-Scientists: A General and End-to-End Agent Harness for Solving Complex Data Visualization Tasks / 迈向 AI VIS 联合科学家：解决复杂数据可视化任务的通用端到端代理工具
...
## 24. SMDD-Bench: Can LLMs Solve Real-World Small Molecule Drug Design Tasks? / SMDD-Bench：法学硕士可以解决现实世界的小分子药物设计任务吗？
