...
大型语言模型 (LLM) 代理越来越多地在多代理环境中充当人类代表，在这种环境中，代表代理会整合不同的同行观点来做出最终决策。从社会心理学中汲取灵感，我们研究了该代表代理人的可靠性如何因其网络的社会背景而受到破坏。我们定义了四个关键现象——社会整合、感知专业知识、主导演讲者效应和修辞说服——并系统地操纵对手的数量、相对智力、论证长度和论证风格。我们的实验表明，随着社会压力的增加，代表代理人的准确性持续下降：更大的对抗群体、更有能力的同伴以及更长的争论都会导致性能显着下降。此外，根据上下文，强调可信度或逻辑的修辞策略可能会进一步影响代理人的判断。这些发现表明，多智能体系统不仅对个体推理敏感，而且对其配置的社会动态也敏感，突显了人工智能代表的关键漏洞，这些漏洞反映了人类群体决策中观察到的心理偏见。
...
可重构智能表面 (RIS) 具有为下一代毫米波 (mmWave) 网络设计智能无线电环境的潜力。然而，信道状态信息（CSI）估计的巨大计算开销和集中式优化固有的维数爆炸严重阻碍了实际的大规模部署。为了克服这些瓶颈，我们引入了由分层多代理强化学习（HMARL）架构支持的“无CSI”范式，以控制机械可重构反射表面。通过用可访问的用户定位数据替代基于导频的信道估计，我们的框架利用空间智能进行宏观波传播管理。控制问题被分解为两层神经架构：高级控制器执行时间扩展的、离散的用户到反射器的分配，低级控制器在集中式训练与分散式执行 (CTDE) 方案下自主优化连续焦点。全面的确定性光线追踪评估表明，该分层框架在集中式基线上实现了高达 7.79 dB 的大规模 RSSI 改进。此外，通过消除 CSI 开销，同时保持亚米级定位跟踪误差，该系统表现出强大的多用户可扩展性并保持高度弹性的光束聚焦性能。高保真信号重定向，这项工作为智能无线环境建立了可扩展且经济高效的蓝图。
...
## 38. Synthetic Sandbox for Training Machine Learning Engineering Agents / 用于训练机器学习工程代理的合成沙箱
...
## 39. Full-Duplex-Bench-v3: Benchmarking Tool Use for Full-Duplex Voice Agents Under Real-World Disfluency / Full-Duplex-Bench-v3：现实世界不流畅情况下全双工语音代理的基准测试工具使用
...
## 40. Explainable Autonomous Cyber Defense using Adversarial Multi-Agent Reinforcement Learning / 使用对抗性多代理强化学习的可解释自主网络防御
...
## 41. Finite-Time Analysis of Q-Value Iteration for General-Sum Stackelberg Games / 一般和 Stackelberg 博弈 Q 值迭代的有限时间分析
...
## 42. Gradual Cognitive Externalization: From Modeling Cognition to Constituting It / 渐进的认知外化：从认知建模到认知构建
...
## 43. How Well Do Agentic Skills Work in the Wild: Benchmarking LLM Skill Usage in Realistic Settings / 代理技能在野外的表现如何：在现实环境中对法学硕士技能的使用进行基准测试
