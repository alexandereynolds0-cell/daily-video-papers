...
## 1. AutoDesign: Meta-Harness Optimization for Long-Horizon Agentic Design / AutoDesign：长期代理设计的元线束优化
...
多智能体系统的组合可靠性界限会增加组件的可靠性，这是一个由条件独立假设许可的步骤，该假设被例行陈述且很少进行测试。我们测试一下。在对 18,000 个任务进行预先注册的评估中，在没有 LLM 法官的情况下，通过确定性代码对 18,000 个任务进行评分，在双智能体切换中，一个模型的两个实例在其中一个失败的任务中 90.0% 共同失败（log OR 6.66，95% CI [6.38, 7.00]；phi 0.916）。替换不同的模型会减少六个对比中的六个的关联性；替换不同的供应商，模型已经不同，不会——注册的假设报告为空。   该错误是有符号的，并且对操作员不利：正相关性将联合故障夸大到独立乘积之上，因此当组件共享模型时，冗余度会被过分夸大。无假设的替代方案通常是空洞的，并且拟合依赖模型更糟糕：我们证明，随着 n 的增长，拟合模型函数上的引导程序界限会失去对事实的覆盖，识别间隙为 O(1)，而引导程序理发为 O(n^{-1/2})。更多数据会使此类证书变得更糟，没有明显的症状。   我们给出了一个假设没有依赖结构的有限样本证书：在联合执行时刻周围的 Bonferroni-Clopper-Pearson 盒上的线性程序。它所提供的信息是健全的、尖锐的，但在当下的家庭中却是单调的。将 10 个矩泛函丰富到 14 个可将确定的区间缩小 85.7%，并将认证下限从 0.2455 提升到 0.4116。配套的随时有效的证书在可选停止下保持 I 类错误为 0.0471。   常见的依赖性统计数据是有边际限制的，并且当比较的代理以不同的速率失败时，可以反转条件的明显顺序。合约、评分代码、分析脚本、预注册均已发布。
...
发现——提出新颖的概括——是科学过程的核心部分。尽管它很重要，但当前的人工智能基准测试领域仍存在差距，很少有基准测试直接探讨在目标未知的受控环境中通过实验发现新知识的能力。为了弥补这一差距，我们发布了一个新的基准测试：DiG-bench（游戏中的发现）。 DiG-bench 由一组 70 个独立游戏组成。每个游戏都被编码为一个短字符串，并且具有必须通过交互和实验来发现的独特转换规则。游戏的关卡提出了一系列的挑战来测试规则是否被发现，而每个关卡的获胜条件也是未知的。我们为人工智能代理提供七个难度级别的游戏。最低层通常可以通过多个模型来解决，而最高层则挑战代理安全带中的最佳模型。所有 70 场比赛均由至少一个人一次性解决。其中 21 款游戏的一部分是公开发布的，其余的则保密以进行安全评估。
...
## 29. AVA-Encoder: Towards Agent-Native Video Representation Learning / AVA-Encoder：迈向代理原生视频表示学习
...
## 30. Scalable Multi-Agent Maze Traversal with Local Communication / 具有本地通信的可扩展多代理迷宫遍历
...
## 31. Total Recall at What Cost? Benchmarking the Serving Cost of Agentic Memory Systems / 全面召回要付出什么代价？对代理内存系统的服务成本进行基准测试
