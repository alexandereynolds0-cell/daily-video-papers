...
## 5. UltraQuant: 4-bit KV Caching for Context-Heavy Agents / UltraQuant：适用于上下文繁重代理的 4 位 KV 缓存
...
我们提出了一种基于模型、基于 RAG 的人工智能经济学家，其具有使用大型语言模型 (LLM) 和知识图进行经济情景分析的代理框架。 While LLMs can generate fluent economic narratives, economists are often required to make economic claims grounded by economic theory and real-world data.基于这一动机，本研究提出了一种基于RAG的人工智能经济学家，它利用包括经济数据和理论在内的知识图谱和基于LLM的代理来规划分析、检索相关证据、选择适当的模型并生成报告。在我们的框架中，我们不会仅使用语言模型直接产生定量主张； instead, we generate narratives grounded in explicit model-based computations and linked to the retrieved evidence via AI agents.我们将我们的框架称为人工智能经济学家代理。我们在两个应用中评估人工智能经济学家代理：针对美国通胀持续性和美联储政策生成经济学家报告，以及针对美国商业房地产再融资压力生成银行压力测试叙述。结果说明了生成的报告的基础如何提高其经济一致性和可追溯性。
...
具有跨不同任务的不同功能的 LLM 代理的分散部署激发了跨异构代理群体的知识共享基础设施。正如搜索引擎索引人类生成的工件以支持人类解决问题一样，检索系统可以组织代理生成的工件以便在代理群体中重用。我们将检索增强生成（这向个体代理展示了人类创作的工件的价值）扩展到支持代理群体的代理生成的工件的检索。特别是，代理轨迹编码可重用的程序知识，但这些工件通常在单次使用后被丢弃或仅由生产代理保留，迫使新实例化的代理重复地重新发现现有解决方案。我们提出了多智能体交互记忆（MATM），这是一个用于群体级存储和检索智能体生成的轨迹的框架，其中生产者智能体将轨迹贡献到共享存储库，消费者智能体检索它们以改善任务执行。 We focus on interactive environments (ALFWorld and WebArena), where trajectories are long and encode especially rich procedural structure.我们的实验表明，从 MATM 检索轨迹可以提高下游任务性能，并减少无需协调或联合训练的交互步骤。这些结果将 MATM 定位为开放代理生态系统中群体级经验共享的设计模式。
...
本文解决了一个迅速出现的政策挑战：如何生成和解释关于人工智能科学家或能够自主或协作执行多步骤科学任务的代理人工智能系统的生物能力和风险的可靠证据。随着这些系统进入真正的研究工作流程，决策者越来越多地面临评估结果，其含义取决于通常是隐含的或记录不足的底层设计选择。我们综合了人工智能驱动的生物风险的当前证据，并引入生物代理评估作为评估这些系统的一种有前途但解释敏感的工具。 Our central contribution is a set of practical, experience-grounded considerations -- drawing from our own evaluations -- that show how choices around defining, designing, running, scoring, and documenting evaluations materially shape what results do and do not imply about risk.该分析旨在帮助政策制定者以适当的谨慎态度解释生物学评价结果；引导公共和私人资助者对人工智能生物学评估研究进行高杠杆投资；并支持生物安全从业者评估新兴人工智能系统。第二受众包括在前沿人工智能实验室、人工智能提供商、科研机构和第三方评估组织内设计或进行代理评估的研究人员。
...
从病理报告中提取信息对于癌症分期、肿瘤登记人群至关重要。然而，关键数据仍然嵌入在叙述性报告中，使得手动提取劳动密集型且容易出错。传统的有监督的自然语言处理管道通过完全监督的命名实体识别和关系提取来解决这个问题，但需要昂贵的手动注释，并且当上游实体丢失时会遭受级联故障。在这项研究中，我们开发了一个零样本、代理工作流程，并评估了五个开源生成大型语言模型 (LLM)，以填充肺切除病理报告中的 13 个美国病理学家学院天气字段。我们使用新颖的、与注册中心一致的评估框架将它们与最先进的监督 GatorTron NER-RE 基线进行比较。基线达到了 0.960 的 Micro-F1，而最佳零样本模型 (GPT-OSS-20B) 达到了 0.893（召回率：0.949）的 Micro-F1，无需特定任务训练即可准确提取病理阶段等复杂关系。 These results suggest that open-source, zero-shot agentic LLMs are a low-cost solution for extracting lung pathology information.
...
电子设计自动化 (EDA) 本质上是多阶段且需要大量切换的。在最终实施、签核或发布之前，设计工件、流程脚本和工程决策会跨工具、会话和组织边界。每个传输都带有显式和隐式的要求，这些要求可能无法被阶段本地检查完全捕获。基于 LLM 的代理现在可以直接调用 EDA 工具，将检索到的知识嵌入可执行脚本中，并跨会话和阶段传递状态。一旦它们的输出影响下游工程决策，转移的对象必须满足移交合同并满足其下一个消费者的假设。本次调查引入了交接有效性作为其组织原则。 A handoff is valid when the transferred object satisfies the consumer's acceptance conditions and carries sufficient context, evidence, and provenance for downstream use.我们回顾了 82 个系统并将它们分为三个边界类别。阶段限制系统在单个 EDA 阶段或有界验证任务内建立有效性。流程绑定系统在工具、调用和会话之间保持一致的工作流程状态。组织约束系统跨知识和权限边界维护源基础、出处、范围和可接受性。对于每一类，我们都会分析切换契约、切换对象、协调机制和开放性问题。这些分析激发了五层 EDA 代理通信协议 (EACP)，涵盖代理发现、代理消息、工具调用、工作流编排以及安全和 IP 协议。我们的目标是为值得信赖的代理 EDA 提供通用词汇和研究议程。
