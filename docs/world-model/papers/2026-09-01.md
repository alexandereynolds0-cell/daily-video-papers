...
## 4. The Intervention Gap in Latent World Models / 潜在世界模型中的干预差距

**Date**: 2026-08-30 | **arXiv**: [2608.29998v1](http://arxiv.org/abs/2608.29998v1) | **PDF**: [Link](http://arxiv.org/pdf/2608.29998v1)
...
规划-时间干预保真度是学习世界模型的一个独特的、可测量的属性：模型自身的开环转换是否像匹配的环境干预那样移动任务变量。 在我们测试的设置中，它既不会通过奖励匹配来显示，也不会通过任务锚定培训来确保。 在发布的TD-MPC2检查点大小中，随着操作员对任务可观察对象的错误诊断的增加，发作回报下降，而奖励预测误差保持较小且接近平坦，并且在没有任务信号的情况下训练的自我监督世界模型大大优于共享任务上的任务锚定模型。 然后，捕获门控匹配干预审核将失败的内容本地化。 在猎豹上，三个LeWorldModel检查点捕获当前任务查询并支持可解码的实际干预效果；然而，它们想象的五步效果比预测无效果更糟糕，比环境端点预言机更糟糕。 故障是具有超额增益的任务方向旋转，而不是特征崩溃。 这种严重的模式是有条件的：五个PreJEPA种子在没有它的情况下保留了甲骨文相对缺陷， Finger Spin实验将缺陷扩展到种子之间的异质性严重程度，并且共享库效应几何形状是候选和支持依赖性的。 我们还会测试练习题。 在DreamerV3中，后验分布（而非其样本）承载当前查询；集成分歧仅在训练附近对错误进行排序

</details>

---

## 5. AcrossWAM1.0:A Modular Latent World-Action Stack for Compact Robot Policies / AcrossWAM1.0 ：紧凑型机器人策略的模块化潜在世界动作堆栈
...
## 6. EMERGE-Policy: A Robot Mind Emerges Beyond a Single Policy / EMERGE-政策：机器人思维超越单一政策
...
## 7. Self-Aware Active Learning Enables Continual Improvement in Autonomous Driving / 自我意识主动学习可实现自动驾驶的持续改进
...
## 8. Does Latent Planning Survive Point Clouds? Action-Conditioned JEPA World Models for Geometric Observations / 潜在规划能否在点云中生存？动作条件JEPA世界几何观测模型
...
JEPA世界模型使潜在空间规划成为一种实用的控制路线，但它们几乎完全建立在图像上。 潜伏预测在几何观测中是否存在尚不清楚：点云稀疏、无序、自遮挡， 0.3-15%的场景点移动，具有三维自监督几何快捷方式的潜伏预测化合物的慢特征优化。 我们将三个规范的JEPA设计提升到点云、冻结编码器、分布优先和动作敏感，并重新感知稳定世界模型基准，以便只有观察结果与图像基线不同。 所有三个计划都不会崩溃：分布优先模型在统计上等同于其在每个基准上重新评估的图像对应物，并且动作敏感模型在我们的受控比较中获得最强的结果，其中大多数几何体移动。 探查解释了原因：物体位置几乎是完全线性解码的，注意力落在几个移动点上。 规划经受住了训练中从未见过的严重辍学，尽管距离噪音击败了最薄的场景。 几何最终使命令3D目标成为自然目标界面：我们从目标和当前潜在目标构建潜在目标，成功率无需成本，无需目标观察。

</details>

---

## 9. AnyWorld: Factorized Egocentric World Models for Cross-Embodiment Generalization / AnyWorld ：跨实体泛化的因子自我中心世界模型
...
大规模收集富有接触的机器人体验仍然是可推广操作的主要瓶颈。 除了数据量之外，机器人学习还需要跨越实施例、观点和场景的多样化体验。 人类以自我为中心的视频提供了丰富的物理互动，但每个视频只能捕捉到单个身体、摄像头轨迹和环境下的狭窄体验。 我们提出了AnyWorld ，这是一个跨实体的世界建模框架，可以将单个人类交互扩展到各种机器人原生推出，而无需人机配对演示。 我们的模型将交互分解为动作、摄像头和实施例：动作控件捕获运动结构，摄像头控件指定视点演化，目标实施例上下文定义作用体及其交互几何形状。 该公式实现了实施例、视点和场景因素的独立重组，允许单个模型生成许多机器人领域体验，同时保留底层动态和对象交互。 我们使用大规模人机交互预训练训练模型，然后进行混合实施例微调。 实验表明，我们的模型支持跨实施例、视点和场景的可控重构，并且我们进一步证明，生成的数据可以提高RoboCasa GR1桌面基准测试和真实铁人形机器人的操作性能。 除了总收益外，我们还测试是否未配对
