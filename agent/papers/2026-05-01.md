...
强化学习 (RL) 已成为大型语言模型 (LLM) 推理、代理能力和对齐后训练的关键。成功的强化学习依赖于模型在训练期间对各种行为的充分探索，这会产生潜在的失败模式：模型可以在训练期间策略性地改变其探索，从而影响后续的训练结果。在本文中，我们研究了这种称为探索黑客的行为。首先，我们通过微调 LLM 来遵循特定的表现不佳策略，创建选择性 RL 抵抗的模型生物；这些模型可以成功地抵抗我们在代理生物安全和人工智能研发环境中基于强化学习的能力启发，同时保持相关任务的性能。然后，我们使用模型生物来评估检测和缓解策略，包括监测、重量噪声和基于 SFT 的启发。最后，我们表明，当提供有关训练环境的足够信息时，当前的前沿模型可以表现出抑制探索的明确推理，而当通过环境间接获取这些信息时，抑制率更高。总之，我们的结果表明，在能力足够强的法学硕士上，探索黑客攻击是强化学习的一种可能失败模式。
...
在本文中，创建、测试和分析了吸引子 FCM。这个 FCM 既不是赫布基础的，也不是代理的，也不是混合体；相反，它是基于梯度下降、受物理约束的 FCM 的雅可比版本。此外，这个模型有几个怪癖：它使用剩余内存、时间反向传播以及递归实现的定点锚点来更新其权重。残差更新递归部分而不丢失系统内存。该模型的锚点使其能够收敛于一个固定点，通过时间反向传播将其展开，并确保误差最小化以获得准确的梯度。此外，还使用了一种新的学习算法。牛顿法找到系统的不动点吸引子，然后梯度下降自适应地改变景观；自适应项用于通过吸引子动力学直接操纵权重。随着自适应项的变化，通过景观的下降会根据 sigmoid 饱和度不断调整，这可以防止过早收敛到局部最小值。最后，更新通过因果掩码进行过滤，告知网络有关物理的信​​息，尊重最初基于专家的意见，为此模型以有效的方式减少目标的误差。
...
大型语言模型 (LLM) 的出现极大地改变了软件工程的任务。在业务流程管理的背景下，法学硕士现在正在被探索作为直接从文本描述导出流程模型的工具。现有的方法包括从协助迭代、基于文本的建模的聊天机器人驱动系统到完全自动化的端到端建模助手。然而，我们认为流程建模本质上很复杂，无法通过黑盒解决方案有效解决。相反，我们将建模视为一种开放式对话活动，最好由涉及人类和法学硕士的交互式迭代过程提供支持。在我们的方法中，建模任务被分解为更小的、可管理的步骤。每个步骤都会产生中间工件，并明确记录每个建模决策背后的基本原理。在此过程中，我们逐渐发现指导模型构建的简单行为关系。鉴于目前法学硕士在推理复杂依赖关系方面的局限性，我们用该领域开发的专门工具来补充它们，以根据行为关系构建流程模型。这种混合方法能够生成健全且易于理解的模型，并通过透明且可解释的步骤不断发展。在本文中，我们提出了我们的研究议程并介绍了 Pragmos，这是一个可实现这一愿景的原型系统。 Pragmos 演示了法学硕士如何与作为领域专家和建模专家的人类用户协作，通过结构化且可解释的工作流程共同创建不断发展的流程模型。

</details>

---

## 25. METASYMBO: Multi-Agent Language-Guided Metamaterial Discovery via Symbolic Latent Evolution / METASYMBO：通过符号潜在演化进行多智能体语言引导的超材料发现
...
超材料发现寻求其几何形状诱导目标机械行为的微结构材料。现有的逆向设计方法可以有效地生成候选方案，但它们通常需要明确的数值属性目标，并且不太适合早期探索，研究人员通常从不完整的约束和用自然语言表达的定性意图开始。大型语言模型可以解释此类意图，但它们缺乏几何意识和物理属性有效性。为了解决这一差距，我们提出了 MetaSymbO，这是一种多智能体框架，用于通过符号驱动的潜在进化来进行语言引导的超材料发现。具体来说，MetaSymbO 包含三个代理：一个设计器，用于解释自由形式的设计意图并检索语义一致的支架；一个生成器，用于在解开的潜在空间中合成候选微结构；以及一个监督器，用于为迭代细化提供快速的属性感知反馈。为了超越从文献和训练数据中再现已知样本的限制，我们进一步引入了符号驱动的潜在进化，它将可编程算子应用于解开的潜在因子，以在推理时组成、修改和细化结构。大量实验表明，(i) 与最先进的基线相比，MetaSymbO 在对称性方面提高了高达 34% 的结构有效性，在周期性方面提高了近 98%； (ii) 与高级推理法学硕士相比，MetaSymbO 的语言指导分数提高了约 6-7%，同时保持了卓越的结构新颖性； (iii) 定性分析证实了符号逻辑运算符在实现可编程语义对齐方面的有效性； (iv) 关于拉胀、高刚度超材料设计的真实案例研究进一步验证了其实用能力。

</details>

---

## 26. The Inverse-Wisdom Law: Architectural Tribalism and the Consensus Paradox in Agentic Swarms / 逆智慧法则：建筑部落主义和代理群体中的共识悖论
