...
新兴的代理网络设想人工智能代理能够通过与现有网络服务交互来可靠地完成用户基于自然语言（NL）的任务。然而，现有的授权模式与这一愿景不一致。特别是，今天的操作员范围授权（以 OAuth 为例）授予与操作员（例如，转账操作员）相关的广泛权限，而不是与用户任务所暗示的特定操作（例如，向 Bob 转账 100 美元）相关的权限。这将不可避免地导致代理人特权过高。   我们引入了精确任务范围隐式授权 (PAuth)，这是一种完全不同的模型，其中提交 NL 任务隐式仅授权其忠实执行所需的具体操作。为了使这一点在服务器上可执行，我们提出了 NL 切片：每个服务期望的调用的符号规范，源自任务和上游结果。作为补充，我们还提出了信封：将每个操作数的具体值与其符号来源绑定的特殊数据结构，使服务器能够验证所有操作数是否来自合法计算。   PAuth 的原型是在代理安全评估框架 AgentDojo 中。我们在良性设置和攻击场景中对其进行评估，其中将虚假操作注入到其他正常任务中。在所有良性测试中，PAuth 无需任何额外权限即可成功执行任务。在所有攻击测试中，PAuth 正确地发出有关缺少权限的警告。这些结果表明 PAuth 关于权限的推理确实是精确的。我们进一步分析这些任务的特征并衡量相关的代币成本。
