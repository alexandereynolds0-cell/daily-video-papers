...
## 10. Bridging Large-Model Reasoning and Real-Time Control via Agentic Fast-Slow Planning / 通过代理快慢规划衔接大模型推理和实时控制
...
科学发现因碎片化的文献而减慢，需要人类付出过多的努力来收集、分析和理解。人工智能工具（包括自主总结和问答）的开发是为了帮助理解科学文献。然而，这些工具缺乏从科学文献中提取深刻见解所需的结构化、多步骤方法。大型语言模型（LLM）为文献分析提供了新的可能性，但由于幻觉和不完整的提取，仍然不可靠。我们推出 Elhuyar，这是一个多智能体、人机交互系统，它集成了法学硕士、结构化人工智能和人类科学家，以从科学文献中提取、分析和迭代提炼见解。该框架将任务分配给专门代理来过滤论文、提取数据、拟合模型和总结结果，并通过人工监督确保可靠性。该系统生成包含提取数据、可视化、模型方程和文本摘要的结构化报告，从而通过迭代细化进行更深入的查询。它应用于材料科学，分析了氦离子辐照下钨的文献，显示了实验上氦气泡的指数增长与辐照剂量和温度的相关性，为聚变反应堆中面向等离子体的材料（PFM）提供了见解。这展示了人工智能辅助文献综述如何揭示科学模式并加速发现。
