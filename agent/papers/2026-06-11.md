...
现实世界的多智能体系统，从交通协调到资源分配，通常被建模为个人激励与集体福利发生冲突的一般和博弈。在这些情况下，核心挑战不仅仅是找到均衡，而是在许多次优纳什均衡中选择社会理想的结果。标准的深度多智能体强化学习（MARL）方法正在努力解决这个问题，因为价值分解方法受到单调性假设的限制，而政策梯度方法通常会收敛到稳定但社会效率低下的均衡。为了解决这一限制，我们提出了 $Φ$-Actor-Critic ($Φ$-AC)，这是一个利用交换遗憾最小化来引导学习走向高福利相关均衡 (CE) 的框架。为了使深度 MARL 中的反事实遗憾估计易于处理，$Φ$-AC 采用了集中式注意力批评器，可以在单次前向传递中预测向量值遗憾，从而避免计算成本高昂的反事实模拟。我们进一步引入了基于拉格朗日的均衡选择机制，该机制优化社会福利，同时通过遗憾约束增强稳定性。矩阵博弈、多智能体粒子环境（MPE）和熔炉收获场景的实验表明，$Φ$-AC 可以在不同的混合动机环境中学习高效稳定的协调策略，同时保持较高的集体回报和竞争公平性。

</details>

---

## 36. Workflow-GYM: Towards Long-Horizon Evaluation of Computer-use Agentic tasks in Real-World Professional Fields / Workflow-GYM：对现实世界专业领域中计算机使用代理任务的长期评估
...
## 37. MASK: Multi-Agent Semantic K-Scheduling for Risk-Sensitive 6G Robotics / MASK：风险敏感的 6G 机器人的多智能体语义 K 调度
