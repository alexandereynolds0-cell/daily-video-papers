...
## 2. Delegation Asymmetry in Agentic Recommender Systems: Measuring Two-Sided Receptivity in Online Dating / 代理推荐系统中的委托不对称性：测量在线约会中的双方接受度
...
## 6. The Curious Case of Exploding DecPOMDPs: Containing the Fire through Policy Counting / DecPOMDP 爆炸的离奇案例：通过政策计数遏制火势
...
学术论文是科学知识的主要载体，但大部分知识仍然锁定在 PDF 中，这些 PDF 已针对人类阅读而不是机器使用进行了优化。对于多模态大型语言模型 (MLLM)，核心挑战不仅在于感知，还在于表示：科学页面将文本与表格、公式、图表和伪代码等结构化学术元素 (SAE) 交织在一起，而 Markdown 等常见替代品很难保留其结构、数据和逻辑。因此，我们提出了可编译学术文档解析（CADP），这是一种将整个页面重建为上下文 \LaTeX{} 加上可执行 Python 的范例，以便可以重建、重新编译结构保留元素和可执行图表表示，并直接根据源页面进行验证。为了支持这种设置，我们引入了 CADP-Bench，这是一个经过专家验证的完整学术页面基准，包含紧密耦合的文本和多种 SAE 类型，并通过重新注入编译协议进行评估。我们使用 SOTA MLLM 和结合了常见代理技术的探索性多代理基线进一步研究当前的功能。结果表明，即使是前沿模型仍然难以产生高保真可执行重建，这凸显了结构感知科学文档解析的巨大改进空间。 CADP-Bench 已发布用于未来研究。
