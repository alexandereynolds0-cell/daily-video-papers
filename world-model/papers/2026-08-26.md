...
世界模型可以预测视频，而无需学习它们可靠保存的动态。我们测试仅在钟摆视频上训练的冷冻DreamerV3是否学习其自身潜在跃迁视为近似守恒的标量。无标签搜索在独立训练的保守模型中恢复相同的能量不变量，而相同的过程在匹配的阻尼模型中找不到可比较的不变量。在自主推出期间，这个数量会随之漂移。将潜伏状态投影回其初始水平集可以减少所有三个保守模型中的推出误差，而匹配的随机约束通常会增加推出误差。这些结果将动态有意义的不变量与仅可解码的相关性区分开来，并揭示了具体的故障模式：世界模型可以从像素中学习物理约束，但在想象向前时违反了该约束。
